...
const { createToken, hashPassword, comparePasswords } = require("../auth/auth");
...
router.get("/", verifyToken, verifyToken, authenticate, async (req, res) => {
  try {
...
router.get("/:id", verifyToken, verifyToken, authenticate, async (req, res) => {
  try {
